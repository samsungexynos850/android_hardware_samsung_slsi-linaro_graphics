...
bool ExynosMPP::isCSCSupportedByMPP(int __unused src_format, int __unused dst_format, uint32_t __unused dataSpace)
{
    return true;
}
...
int ExynosMPP::processM2M(hwc_layer_1_t &layer, int dstFormat, hwc_frect_t *sourceCrop, bool __unused needBufferAlloc)
{
    ...
    if (needBufferAlloc) {
        ...
        if (!reconfigure &&
            (mDisplay != NULL) && (mDisplay->mHwc->hwc_ctrl.skip_m2m_processing_mode != 0) &&
